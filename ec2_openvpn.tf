...
resource "null_resource" "ansible" {
  ...
  provisioner "local-exec" {
    command = "./files/update_ansible.sh ${var.ec2_openvpn["porta_ssh"]} ${var.ec2_openvpn["porta_openvpn"]} ${aws_eip.openvpn.public_ip} '${var.openvpn["KEY_COUNTRY"]}' '${var.openvpn["KEY_PROVINCE"]}' '${var.openvpn["KEY_CITY"]}' '${var.openvpn["KEY_ORG"]}' '${var.openvpn["KEY_EMAIL"]}'"
  }

  provisioner "local-exec" {
    # O sleep eh por conta de um delay que tem apos criar a instancia e aplicar o
    # user_data
    command = "sleep 30; cd ansible; ansible-playbook -i iventory/aws/ pb_openvpn.yml ; cd .."
  }
}
